...
const db = Massive.connectSync({db: 'cesargarcia'});
...
app.post("/charge", (req, res) => {
    ...
    stripe.customers.create({email: req.body.email, source: req.body.id}).then(customer => stripe.charges.create({amount: req.body.amount, description: "Sample Charge", currency: "usd", customer: customer.id})).then(charge => res.send(charge));
    ...
    db.findUserByEmail([req.body.email], (err, result) => {
        ...
        if (result[0]) {
            db.createDonation([
                realAmount, req.body.email, result[0].id
            ], (err, donation) => {})
        } else {
            db.createUser([
                ...
                console.log(user)
                db.createDonation([
                    realAmount, req.body.email, user[0].id
                ], (err, newUser) => {})
            });
        }
    })
...
// PUBLISHABLE_KEY=pk_test_ybmsJKzVtoxdhkx7tN6lUjRL SECRET_KEY=sk_test_0l6ZPAhCkVfA9tOIjjNZ0llH nodemon server.js
